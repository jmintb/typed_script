...
impl FunctionDeclaration {
    ...
    pub fn argument_types(&self) -> impl Iterator<Item = &Type> {
        self.arguments.iter().map(|argument| {
            argument.r#type.as_ref().unwrap_or_else(|| {
                panic!(
                    "expected a parameter type in function {:?}",
                    self.identifier
                )
            })
        })
...
impl Expression {
    ...
    pub fn to_debug_string(&self, db: &NodeDatabase) -> Result<String> {
        Ok(match self {
            Expression::Value(value) => value.to_debug_string(),
            ...
            Expression::Call(call) => {
                let arguments: Vec<String> = call
                    .arguments
                    .iter()
                    .map(|id| db.expressions.get(id).unwrap().to_debug_string(db).unwrap())
                    .collect();
                format!("call {}({:?})", call.function_id.0, arguments)
            }
            Expression::Operation(operation) => operation.to_debug_string(db)?,
            ...
            Expression::ArrayLookup(array_lookup) => {
                let index_debug_string = db
                    .expressions
                    .get(&array_lookup.index_expression)
                    .unwrap()
                    .to_debug_string(db)?;
                format!(
                    "arraylookup({}, {})",
                    array_lookup.array_identifier.0, index_debug_string
                )
            }
    ...
    pub fn to_debug_string(&self, db: &NodeDatabase) -> Result<String> {
        match self {
            Self::Binary(lhs, operator, rhs) => {
                ...
                Ok(format!(
                    "Operation({lhs_debug_string} {operator_debug_string} {rhs_debug_string})"
                ))
            }
        }
    }
