                ...
                for (instruction_count, instruction) in block.instructions.iter().enumerate() {
                    f.write_fmt(format_args!(
                        ...
                        instruction.to_display_string(
                            &self.ssa_variables[&function_id],
                            &self.static_values
                        )
                    ))?;
                }
    ...
    pub fn to_display_string(
        ...
        match self {
            Self::WhileLoop {
                ...
                format!("While block_{:?}  {{ block_{:?} }}", condition, body)
            }
            Self::ArrayLookup {
                array,
                index,
...
impl IrGenerator {
    ...
    fn convert_block_from_control_flow(
        &mut self,
        ast_block_id: BlockID,
        current_block: BlockId,
    ) -> BlockId {
    ...
    fn convert_expression(
        ...
        match expression {
            ...
            Expression::If(IfStatement {
                ...
                let (condition_block, condition_result) =
                    self.convert_expression(condition, current_block);
                let converted_then_block =
                    self.convert_block_from_control_flow(then_block, condition_block); // TODO: this is a hack it should not be necessary to mark a block.
                let post_if_block = self.add_block();
                self.record_cfg_connection(converted_then_block, post_if_block);
                let if_instruction =
                    Instruction::If(condition_result.unwrap(), converted_then_block);
            ...
            Expression::Ifelse(IfElseStatement {
                ...
                let parent_block = current_block;
                let (inner_current_block, condition_ssaid) =
                    self.convert_expression(condition, current_block);
                ...
                let if_else_instruction = Instruction::IfElse(
                    condition_ssaid.unwrap(),
                    converted_then_block,
                    converted_else_block,
                );
            ...
            Expression::Assign(Assign { id, expression }) => {
                current_block = self.convert_assign(Assign { id, expression }, current_block);
            }
            ...
            Expression::ArrayLookup(ArrayLookup {
                ...
                let (index_expression_block, index_ssaid) =
                    self.convert_expression(index_expression, current_block);
                ...
                let result_ssa_id =
                    self.add_ssa_variable(Identifier::new("@array_lookup_result".to_string()));
