...
pub fn evaluate_types(ir_program: &IrProgram) -> Result<BTreeMap<FunctionDeclarationID, IrProgramTypes>> {
    ...
    for function_id in ir_program.control_flow_graphs.keys() {
        let ir_interpreter: IrInterpreter<'_, IrProgramTypes> = IrInterpreter::new(ir_program.control_flow_graphs.get(function_id).unwrap(), ir_program);
    ...
    fn check_types(instruction_counter: usize, ctx: &mut TransformContext, block_id: &BlockId,  bc_ctx: &mut IrProgramTypes) -> Result<usize> {
        ...
        match instruction {
            Instruction::InitArray(elements, receiver) => {
                ...
                let array_type = ArrayType { element_type, length: elements.len() };
                let type_id = bc_ctx.array_types.insert(array_type);
                bc_ctx.variable_types.insert(*receiver, Type::Array(type_id));
            ...
            Instruction::ArrayLookup { array, result, .. } => {
                ...
                let array_type = bc_ctx.array_types.get(*array_variable_type_id).expect("type must exist at this point");
                bc_ctx.variable_types.insert(*result, array_type.element_type);
                debug!("set type {:#?} for variable {}", array_type.element_type, result.0);
            }
            _ => ()
        }
